...
public class ElasticSearchTest {
    ...
    public void testEventsForUser() {
        ...
        SearchResponse response = client.matchEvents("alfresco", "admin", 0, 300);
        for (SearchHit hit : response.getHits().hits()) {
            String nodeId = null;
    ...
    public void testMatchEventsForNode() {
        ...
        SearchResponse response = client.matchEventsForNode("alfresco", "7726cb91-ba78-4554-8c15-e2e3de2127d0", 0, 300);
        for (SearchHit hit : response.getHits().hits()) {
            String nodeId = null;
            String timestamp = null;
    ...
    public void testUserAggregation() throws Exception {
        ...
        TermsBuilder aggregationBuilder = AggregationBuilders.terms("users_agg").field("u")
        ...
        for (Terms.Bucket bucket : buckets) {
            ...
            SearchResponse response1 = client.search("alfresco", queryBuilder1, null, Arrays.asList(IndexType.node),
                    new String[] { "id", "n", "v" });
    ...
    public void testUsernameAggregations() throws Exception {
        ...
        DateHistogramBuilder aggregationBuilder = AggregationBuilders.dateHistogram("event_dates_agg").field("ti")
                .interval(1000 * 60 * 60).subAggregation(AggregationBuilders.terms("event_usernames").field("u"));

        SearchResponse response = client.search("alfresco", queryBuilder, aggregationBuilder,
                Arrays.asList(IndexType.event), new String[] { "t", "u", "ct", "tx", "txnId", "ti", "id", "v", "n" });

        Histogram dateHistogram = response.getAggregations().get("event_dates_agg");
        List<? extends Histogram.Bucket> buckets = dateHistogram.getBuckets();
        for (Histogram.Bucket bucket : buckets) {
            String date = bucket.getKeyAsString();
            long dateBucketCount = bucket.getDocCount();
            logger.debug(
                    "dates histogram bucket " + date + " = " + dateBucketCount);
    ...
    public void testSitesAggregation() throws Exception {
        ...
        TermsBuilder aggregationBuilder = AggregationBuilders.terms("event_sites_agg").field("s");
        ...
        for (Terms.Bucket bucket : buckets) {
            String siteId = bucket.getKeyAsString();
    ...
    public void testEventTypesAggregation() throws Exception {
        ...
        TermsBuilder aggregationBuilder = AggregationBuilders.terms("event_types_agg").field("t");

        SearchResponse response = client.search("alfresco", queryBuilder, aggregationBuilder,
                Arrays.asList(IndexType.event),
                new String[] { "t", "u", "ct", "tx", "txnId", "ti", "id", "v", "n", "ne" });
