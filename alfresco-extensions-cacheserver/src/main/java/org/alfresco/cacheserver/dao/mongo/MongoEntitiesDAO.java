...
public class MongoEntitiesDAO extends AbstractMongoDAO implements EntitiesDAO
{
	...
	@Override
	public Collection<Entity<String>> getNames(NodeInfo node)
	{
		String nodeId = node.getNodeId();
		String nodeVersion = node.getNodeVersion();
	...
	@Override
	public EntityCounts<String> getEntityCounts(NodeInfo node)
	{
		String nodeId = node.getNodeId();
		String nodeVersion = node.getNodeVersion();
...
//				.and("c").is(true)
	...
	@Override
	public List<NodeInfo> matchingNodes(String type, String name)
	{
		...
		try
		{
			for(DBObject dbObject : cursor)
			{
				...
				NodeInfo node = new NodeInfo(nodeId, nodeInternalId, nodeVersion);
				nodes.add(node);
			}
		}
		finally
	...
	@Override
	public Entities getEntities(NodeInfo node, Set<String> types)
	{
		String nodeId = node.getNodeId();
		String nodeVersion = node.getNodeVersion();
