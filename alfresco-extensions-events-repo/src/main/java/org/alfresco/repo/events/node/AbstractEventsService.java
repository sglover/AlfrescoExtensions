...
public abstract class AbstractEventsService extends TransactionListenerAdapter
{
    ...
    public void beforeCompletion()
    {
        ...
        {
            // send all events
            final TxnEvents events = (TxnEvents) AlfrescoTransactionSupport
    ...
    protected Client getAlfrescoClient(Client knownClientType)
    {
        ...
        if (context != null)
        {
            HttpServletRequest request = (HttpServletRequest) context
                    .get(CallContext.HTTP_SERVLET_REQUEST);
            if (request != null)
            {
                alfrescoClientId = (String) request
                        .getHeader("alfrescoClientId");
    ...
    public void afterCommit()
    {
        if (sendEventsBeforeCommit)
        {
            ...
            final TransactionCommittedEvent event = new TransactionCommittedEvent(
                    nextSequenceNumber(), txnGuid, networkId, timestamp,
                    username, alfrescoClient);
            ...
            // Need to execute this in another read txn because Camel/JMS
            // expects it (the config now seems to
            // require a txn)
    ...
    public void afterRollback()
    {
        ...
        final TransactionRolledBackEvent event = new TransactionRolledBackEvent(
                nextSequenceNumber(), txnGuid, networkId, timestamp, username,
                alfrescoClient);
        ...
        {
            TxnEvents events = (TxnEvents) AlfrescoTransactionSupport
                    .getResource(EVENTS_KEY);
            if (events != null)
            {
                events.clear();
    ...
    protected class NodeInfo
    {
        ...
        public NodeInfo(String eventType, String txnId, String name,
                NodeRef nodeRef, long nodeInternalId, long nodeVersion,
                String versionLabel, Status status, List<Path> nodePaths,
                Long modificationTimestamp, QName type, Set<QName> aspects,
                Map<QName, Serializable> properties, Long aclId, String siteId,
                Client client, Boolean nodeExists, boolean include,
        ...
        public Map<String, Serializable> getProperties()
        {
            ...
            {
                for (Map.Entry<QName, Serializable> entry : this.properties
                        .entrySet())
                {
                    QName propQName = entry.getKey();
                    String propName = propQName
                            .toPrefixString(namespaceService);
                    Serializable propValue = entry.getValue();
                    Serializable serializablePropValue = propertySerializer
                            .serialize(propQName, propValue);
        ...
        public boolean checkNodeInfo()
        {
            ...
            if (!nodeExists)
            {
                logger.warn("Unable to send event of type " + eventType
                        + ", node not found: " + this.toString());
                ret = false;
            }
            else if (!include)
            {
                logger.warn("Unable to send event of type " + eventType
                        + ", node not included: " + this.toString());
                ret = false;
            }
            else if (!typeMatches)
            {
                logger.warn("Unable to send event of type " + eventType
                        + ", node type filtered: " + this.toString());
                ret = false;
            }
            else if (!isVisible)
            {
                logger.warn("Unable to send event of type " + eventType
                        + ", node is not visible: " + this.toString());
                ret = false;
            }
        ...
        public List<String> getPaths()
        {
            ...
            {
                stringPaths = AbstractEventsService.this.getPaths(nodePaths,
                        getToAppend());
            }
        ...
        public List<List<String>> getParentNodeIds()
        {
            ...
            {
                parentNodeIds = AbstractEventsService.this
                        .getNodeIdsFromParent(nodePaths);
            }
    ...
    protected List<String> getPaths(List<Path> nodePaths, List<String> toAppend)
    {
        ...
        {
            StringBuilder pathStr = new StringBuilder(path.toDisplayPath(
    ...
    /**
     * For each path, construct a list of ancestor node ids starting with the
     * parent node id first.
     * 
     * @param nodePaths
     * @return
     */
    protected List<List<String>> getNodeIdsFromParent(List<Path> nodePaths)
    {
        ...
        for (Path path : nodePaths)
        {
            ...
            // add in reverse order (so the first element is the immediate
            // parent)
            for (int i = path.size() - 2; i >= 0; i--)
            {
                ...
                if (element instanceof ChildAssocElement)
                {
                    ...
                    NodeRef childNodeRef = childAssocElem.getRef()
                            .getChildRef();
    ...
    /**
     * For each path, construct a list of ancestor node ids starting with the
     * leaf path element node id first.
     * 
     * @param nodePaths
     * @return
     */
        ...
        void sendEvents()
        {
            if (events != null && events.size() > 0)
            {
                try
                {
                    ...
                    {
                        // nasty hack to get around the fact that the
                        // VersionableAspect auto-versions
                        // on transaction commit
                        // TODO optimisation - check if the node is
                        // auto-versioned. If not, this can
                        // be skipped
                        if (event instanceof NodeEvent)
                        {
                            NodeEvent nodeEvent = (NodeEvent) event;

                            Map<String, Serializable> properties = nodeEvent
                                    .getNodeProperties();
                            String versionLabel = (String) properties
    ...
    {
        NodeInfo nodeInfo = AuthenticationUtil
                .runAsSystem(new RunAsWork<NodeInfo>()
                {
                    public NodeInfo doWork() throws Exception
                    {
                        NodeInfo nodeInfo = null;

                        String txnId = AlfrescoTransactionSupport
                                .getTransactionId();
                        Status status = nodeService.getNodeStatus(nodeRef);
                        long nodeInternalId = (status != null ? status
                                .getDbId() : -1);
                        ...
                        {
                            nodeInfo = new NodeInfo(eventType, null, null,
                                    nodeRef, nodeInternalId, nodeVersion, null,
                        ...
                        {
                            nodeInfo = new NodeInfo(eventType, txnId, null,
                                    nodeRef, nodeInternalId, nodeVersion, null,
                                    null, null, null, null, null, null, null,
                        ...
                        {
                            FileFilterMode.Client filterclient = FileFilterMode
                                    .getClient();
                            Visibility visibility = hiddenAspect.getVisibility(
                                    filterclient, nodeRef);
                            QName type = nodeService.getType(nodeRef);

                            if (!typeMatches(type))
                            {
                                nodeInfo = new NodeInfo(eventType, txnId, null,
                                        nodeRef, nodeInternalId, nodeVersion,
                            ...
                            else
                            {
                                SiteInfo siteInfo = siteService
                                        .getSite(nodeRef);
                                String siteId = (siteInfo != null ? siteInfo
                                        .getShortName() : null);
                                Long aclId = nodeService.getNodeAclId(nodeRef);

                                Set<QName> aspects = nodeService
                                        .getAspects(nodeRef);
                                ...
                                final String name = (String) nodeService
                                        .getProperty(nodeRef,
                                                ContentModel.PROP_NAME);
                                List<Path> nodePaths = nodeService.getPaths(
                                        nodeRef, false);

                                Date modifiedTime = (Date) nodeService
                                        .getProperty(nodeRef,
                                                ContentModel.PROP_MODIFIED);
                                Long modificationTimestamp = (modifiedTime != null ? modifiedTime
                                        .getTime() : null);

                                Client client = ClientUtil.from(filterclient);
                                nodeInfo = new NodeInfo(eventType, txnId, name,
                                        nodeRef, nodeInternalId, nodeVersion,
                                        versionLabel, status, nodePaths,
                                        modificationTimestamp, type, aspects,
                                        properties, aclId, siteId, client,
                                        true, true, true, true);
                            }
                        }
