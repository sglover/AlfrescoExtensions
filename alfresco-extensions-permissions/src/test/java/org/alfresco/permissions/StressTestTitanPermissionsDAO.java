 ...
 * @author sglover
 *
 * avg write (ms) 0.324283
 * total write (ms) 3243.1587
 * 100 children in time (ms) 21.16297
 * 100 children in time (ms) 14.405273
 * 100 children in time (ms) 15.215483
 * 100 children in time (ms) 13.529972
 * 100 children in time (ms) 10.233615
 * 1 children in time (ms) 8.228028
 * 1 children in time (ms) 7.112936
 * 1 children in time (ms) 6.339148
 * 1 children in time (ms) 6.151559
 * 1 children in time (ms) 6.736978
 */
	...
	public void test1() throws Exception
	{
...

		{
			for(int i = 0; i < numChildren; i++)
			{
				...
				permissionsDAO.setPermission("parent1", "v1", "child", "Read", "GROUP_1", "child" + i, "v1");
				long end = System.nanoTime();
				total += (end - start);
			}
			long start = System.nanoTime();
			permissionsDAO.setPermission("parent1", "v1", "child", "Read", "cknight", "child" + (numChildren + 1), "v1");
			long end = System.nanoTime();
			total += (end - start);
		}
		...
		for(int i = 0; i < 5; i++)
		{
			...
			assertTrue(children.contains(Node.withNodeId("child" + (numChildren + 1)).withNodeVersion("v1")));
			System.out.println(children.size() + " children in time (ms) " + (end - start)/1000000.0f);
		}
	}
