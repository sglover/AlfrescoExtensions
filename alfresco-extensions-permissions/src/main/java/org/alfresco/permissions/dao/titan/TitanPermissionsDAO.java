 ...
 * 
 * Pre-requisites:
 *  Cassandra: cd /Users/sglover/dev/apache-cassandra-2.1.2; bin/cassandra -f
...
public class TitanPermissionsDAO implements PermissionsDAO
{
	...
	public static class RemoteCassandraConfig implements TitanConfig
	{
		...
		public void apply(TitanFactory.Builder config)
		{
        	...
        	{
    			KeyspaceMetadata keySpaceMetadata = cassandraSession.getCluster().getMetadata().getKeyspace("titan");
	...
	public static class RemoteElasticSearchConfig implements TitanConfig
	{
		...
		public RemoteElasticSearchConfig(String hostname, boolean deleteIndex)
		{
	    	...
	    	org.elasticsearch.node.Node node = nodeBuilder()./*clusterName("elk").*/node();
    ...
    private void load(String mixedIndexName)
    {
    	...
    	TitanGraphIndex byAuthorityName = mgmt
    			.buildIndex("byAuthorityName", Vertex.class).indexOnly(authorityVertexLabel).addKey(name)
    			//.unique()
    			.buildCompositeIndex();
    	...
    	TitanGraphIndex byPermissionName = mgmt
    			.buildIndex("byPermissionName", Vertex.class).indexOnly(permissionVertexLabel).addKey(name)
    			//.unique()
    			.buildCompositeIndex();
    	...
    	final PropertyKey nodeVersion = mgmt.makePropertyKey("nodeVersion").dataType(String.class).make();
    	TitanGraphIndex byNode = mgmt.buildIndex("byNodeId", Vertex.class).addKey(nodeId).addKey(nodeVersion)
    	...
    	TitanGraphIndex propertiesByNode = mgmt.buildIndex("propertiesByNode", Vertex.class).addKey(nodeId).addKey(nodeVersion)
    			.indexOnly(nodePropertiesVertexLabel)
    			.buildCompositeIndex();
    	//.buildMixedIndex(mixedIndexName);
    	mgmt.setConsistency(propertiesByNode, ConsistencyModifier.LOCK);

    	EdgeLabel childrenEdgeLabel = mgmt.makeEdgeLabel("child").make();
    	final PropertyKey permission = mgmt.makePropertyKey("permission").dataType(String.class).make();
    	final PropertyKey authority = mgmt.makePropertyKey("authority").dataType(String.class).make();
    	mgmt.buildIndex("byAuthAndPer", Edge.class).addKey(permission).addKey(authority)
    		.indexOnly(childrenEdgeLabel)
    		.buildCompositeIndex();
...
//
//        List<Vertex> l = graph.traversal().V().hasLabel("PERMISSION").has("name", parentPermission).toList();
//        Vertex parent = null;
//        if(l.size() > 0)
//        {
//        	parent = l.get(0);
...
//        else
//        {
//            parent = graph.addVertex(T.label, "PERMISSION", "name", parentPermission);
//        }
//    }
	...
	{
        Optional<Vertex> parentOpt = graph.traversal().V().hasLabel("PERMISSION").has("name", permission).tryNext();
        Stream<String> s = parentOpt.flatMap(parent ->
        {
    		GraphTraversal<Vertex, Vertex> containedPermissions = graph.traversal().V(parent).repeat(__.out("child")).emit();
    		Stream<String> stream = StreamSupport.stream(Spliterators.spliteratorUnknownSize(containedPermissions,
    ...
    public void addChildPermission(String parentPermission, String childPermission)
    {
		try
		{
	        Optional<Vertex> parentOpt = graph.traversal().V().hasLabel("PERMISSION").has("name", parentPermission).tryNext();
...
//        List<Vertex> l = tx.traversal().V().hasLabel("PERMISSION").has("name", parentPermission).toList();
//        Vertex parent = null;
//        if(l.size() > 0)
//        {
//        	parent = l.get(0);
...
//        else
//        {
//            parent = tx.addVertex(T.label, "PERMISSION", "name", parentPermission);
//        }

//		Iterator it = graph.query().has("type", "PERMISSION").has("name", parentPermission).vertices().iterator();
//		Vertex parent;
//		if(!it.hasNext())
//		{
//			parent = tx.addVertex(T.label, parentPermission, "name", parentPermission, "type", "PERMISSION");
//		}
//		else
//		{
//			parent = (Vertex)it.next();
//		}
//        Optional<Vertex> parentOpt = graph.query().has("type", "PERMISSION").has("name", parentPermission).vertices().iterator().tryNext();
//        Vertex parent = parentOpt.orElseGet(()->
//        {
//            TitanVertex p = tx.addVertex(T.label, parentPermission, "name", parentPermission, "type", "PERMISSION");
////            TitanVertex p = graph.addVertex(T.label, parentPermission, "name", parentPermission, "type", "PERMISSION");
//            return p;
//        });

//        TitanVertex child = tx.addVertex(T.label, childPermission, "name", childPermission, "type", "PERMISSION");
    }

	@Override
    public void addChildAuthority(String parentAuthority, String childAuthority)
    {
//        Vertex parent = g.V().has("type", "Authority").has("name", parentAuthority).next();

		try
		{
	        Optional<Vertex> parentOpt = graph.traversal().V().hasLabel("AUTHORITY").has("name", parentAuthority).tryNext();
	        Vertex parent = parentOpt.orElseGet(()->
	        {
	            TitanVertex p = graph.addVertex(T.label, "AUTHORITY", "name", parentAuthority);
	            return p;
	        });
	//        TitanVertex parent = tx.query().has("name", parentAuthority).vertices().iterator().next();
    ...
    public Stream<String> getChildAuthoritiesAsStream(String parentAuthority)
    {
        try
        {
	        Vertex parent = graph.traversal().V().hasLabel("AUTHORITY").has("name", parentAuthority).next();
	
	        Iterator<Vertex> vertices = parent.vertices(Direction.OUT, "child");
	
			Stream<String> stream = StreamSupport.stream(Spliterators.spliteratorUnknownSize(vertices,
					Spliterator.ORDERED), false)
					.map(v ->
					{
    ...
    {
		Vertex group1 = graph.traversal().V().hasLabel("AUTHORITY").has("name", parentAuthority).next();

		GraphTraversal<Vertex, Vertex> containedAuthorities = graph.traversal().V(group1).repeat(__.out("child")).emit();
		Stream<String> stream = StreamSupport.stream(Spliterators.spliteratorUnknownSize(containedAuthorities,
				Spliterator.ORDERED), false)
				.map(v ->
				{
					return (String)v.property("name").value();
	...
	{
		Optional<Vertex> parentNodeOpt = graph.traversal().V()
				.hasLabel("NODE")
				.has("nodeId", parentNodeId)
				.has("nodeVersion", parentNodeVersion)
				.tryNext();
		List<Node> assocs = parentNodeOpt.flatMap(parentNode ->
			Optional.of(graph.traversal().V(parentNode)
						.outE(assocType)
...
//						.emit(t ->
//						{
//							String val = (String)t.get().property(permission + "." + authority).orElse(null);
...
//								(String)t.get().property("nodeVersion").value()))
	...
	@Override
	public Optional<Properties> getNodeProperties(String nodeId, String nodeVersion, String permission, String authority)
	{
		Optional<Vertex> nodeOpt = graph.traversal().V()
				.hasLabel("NODE")
				.has("nodeId", nodeId)
				.has("nodeVersion", nodeVersion)
				.tryNext();
		Optional<Properties> properties = nodeOpt.flatMap(node ->
		{
			return graph.traversal().V(node)
				.outE("properties")
				.has("authority", authority)
				.has("permission", permission)
				.inV()
				.range(0, 1)
				.map(v -> new Properties(v.get().properties()))
				.tryNext();
		});
    ...
    public List<Node> getChildren(String parentNodeId,
            String parentNodeVersion, String authority, final int skip, final int limit)
    {
		return getAssocs(parentNodeId, parentNodeVersion, "child", "ReadChildren", authority, skip, limit);
    }
	...
	public void setPropertiesPermission(String nodeId, String nodeVersion, String authority)
	{
		...
		Vertex source = graph.traversal().V()
				...
				.orElseGet(() ->
				{
					TitanVertex p = graph.addVertex(T.label, "NODE", "nodeId", nodeId, "nodeVersion", nodeVersion);
					return p;
				});
//		source.property(key, value);

		Vertex target = graph.traversal().V()
				.hasLabel("PROPERTIES")
				.has("nodeId", nodeId)
				.has("nodeVersion", nodeVersion)
				.tryNext()
        ...
        try(Stream<String> containedAuths = Stream.concat(Stream.of(authority), getContainedAuthoritiesAsStream(authority)))
        {
        	containedAuths
        	.flatMap(auth -> containedPerms.stream().<AuthAndPerm>map(perm -> new AuthAndPerm(auth, perm)))
        	.forEach(authAndPerm ->
        	{
	...
	@Override
    public void setPermission(String sourceNodeId, String sourceNodeVersion,
            String assocType, String permission, String authority,
            String targetNodeId, String targetNodeVersion)
    {
		...
		Vertex source = graph.traversal().V()
				.hasLabel("NODE")
				.has("nodeId", sourceNodeId)
				.has("nodeVersion", sourceNodeVersion)
				.tryNext()
				.orElseGet(() ->
				{
					TitanVertex p = graph.addVertex(T.label, "NODE", "nodeId", sourceNodeId, "nodeVersion", sourceNodeVersion);
					return p;
				});

		Vertex target = graph.traversal().V()
				.hasLabel("NODE")
				.has("nodeId", targetNodeId)
				.has("nodeVersion", targetNodeVersion)
				.tryNext()
				.orElseGet(() ->
				{
					TitanVertex p = graph.addVertex(T.label, "NODE", "nodeId", targetNodeId, "nodeVersion", targetNodeVersion);
					return p;
				});
