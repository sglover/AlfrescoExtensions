...
public class CassandraPermissionsDAO implements PermissionsDAO
{
	...
	public CassandraPermissionsDAO(Session session, String keyspace, boolean dropKeyspaceOnInit)
	{
		...
		if(keySpaceMetadata == null)
		{
		    ...
		    session.execute("CREATE TABLE " + keyspace + ".child_permissions (" + "parent_permission text, "
		    		+ "child_permission text,"
		            + "PRIMARY KEY(parent_permission, child_permission) "
		            + ");");

		    session.execute("CREATE TABLE " + keyspace + ".child_authorities (" + "parent_authority text, "
		    		+ "level int,"
		    		+ "child_authority text,"
		            + "PRIMARY KEY(parent_authority, level, child_authority) "
		            + ");");

		    session.execute("CREATE TABLE " + keyspace + ".parent_authorities (" + "child_authority text, "
		    		+ "level int,"
	    ...
	    this.insertChildPermission = session.prepare("insert into " + keyspace + ".child_permissions "
	    		+ "(parent_permission, child_permission) "
				+ "VALUES(?, ?)");
	    this.insertChildAuthority = session.prepare("insert into " + keyspace + ".child_authorities "
	    		+ "(parent_authority, level, child_authority) "
				+ "VALUES(?, ?, ?)");
	    this.removeChildPermission = session.prepare("delete from " + keyspace + ".child_permissions "
	    		+ "WHERE parent_permission = ? AND child_permission = ?");
	    this.removeChildAuthority = session.prepare("delete from " + keyspace + ".child_authorities "
	    		+ "WHERE parent_authority = ? AND level = ? AND child_authority = ?");
	    this.getChildPermissions = session.prepare("select * from " + keyspace + ".child_permissions where parent_permission = ?");
	    this.getDescendantAuthorities = session.prepare("select * from " + keyspace + ".child_authorities where parent_authority = ? AND level > 0");
	    this.getChildAuthorities = session.prepare("select * from " + keyspace + ".child_authorities where parent_authority = ? AND level = 1");
	    this.insertNodeAssoc = session.prepare("insert into " + keyspace + ".node_assocs (source_node_id, source_node_version, "
	    		+ "assoc_type, "
	    		+ "permission, authority, target_node_id, target_node_version) VALUES(?, ?, ?, ?, ?, ?, ?)");
	    this.insertParentAuthority = session.prepare("insert into " + keyspace + ".parent_authorities "
	    		+ "(child_authority, level, parent_authority) "
				+ "VALUES(?, ?, ?)");
	    this.getParentAuthorities = session.prepare("select * from " + keyspace + ".parent_authorities "
	    		+ "WHERE child_authority = ? AND level=1");
	    this.getAncestorAuthorities = session.prepare("select * from " + keyspace + ".parent_authorities "
	    		+ "WHERE child_authority = ?");
	    this.removeParentAuthority = session.prepare("delete from " + keyspace + ".parent_authorities "
	    		+ "WHERE child_authority = ? AND level = ? AND parent_authority = ?");
		this.getNodeAssocs = session.prepare("SELECT * FROM " + keyspace + ".node_assocs WHERE source_node_id=?"
				+ " AND source_node_version=?"
				+ " AND assoc_type=?"
				+ " AND permission='ReadChildren'"
				+ " AND authority=?"
				+ " LIMIT ?"); 
	}
...
//        		.onClose(() -> resultSet.)  // need to close cursor;
	...
	public void addChildAuthority(String parentAuthority, String childAuthority)
	{
		Stream<Map.Entry<Integer, String>> ancestorAuthorities = CollectionUtils.zipWithIndex(
				Stream.concat(Stream.of(parentAuthority), getAncestorAuthorities(parentAuthority)), 1);
		ancestorAuthorities
			.forEach(authLevel ->
			{
				String ancestorAuthority = authLevel.getValue();
				int level = authLevel.getKey();
				BoundStatement childInsert = insertChildAuthority.bind(ancestorAuthority, level, childAuthority);
				session.execute(childInsert);

				BoundStatement parentInsert = insertParentAuthority.bind(childAuthority, level, ancestorAuthority);
				session.execute(parentInsert);
			});
//
//		BoundStatement parentInsert = insertParentAuthority.bind(childAuthority, parentAuthority);
//		session.execute(parentInsert);
	}
	...
	{
		Stream<Map.Entry<Integer, String>> ancestorAuthorities = CollectionUtils.zipWithIndex(
				Stream.concat(Stream.of(parentAuthority), getParentAuthorities(parentAuthority)), 1);
		ancestorAuthorities
			.forEach(authLevel ->
			{
				String ancestorAuthority = authLevel.getValue();
				int level = authLevel.getKey();
				BoundStatement childDelete = removeChildAuthority.bind(ancestorAuthority, level, childAuthority);
				session.execute(childDelete);
				BoundStatement insert = removeParentAuthority.bind(childAuthority, level, parentAuthority);
				session.execute(insert);
			});
	}

	@Override
	public void setPermission(final String sourceNodeId, final String parentNodeVersion, final String assocType,
			...
			final String targetNodeId, final String childNodeVersion)
	{
		...
		Stream<String> authoritiesStream = Stream.concat(Stream.of(authority), getContainedAuthoritiesAsStream(authority));
	...
	public void setSinglePermission(String parentNodeId, String parentNodeVersion, String assocType,
			String permission, String authority,
			String childNodeId, String childNodeVersion)
	{
		BoundStatement insert = insertNodeAssoc.bind(parentNodeId, parentNodeVersion, assocType, permission, authority,
			...
			int skip, int limit)
	{
		...
		BoundStatement boundStatement = getNodeAssocs.bind(parentNodeId, parentNodeVersion, "Children", authority, limit);
...
//        {
//    		setPermissions(parentNodeId, parentNodeId, permissions, childNodeId, childNodeVersion);
//        }
//
//        @Override
