...
public class BitSetCassandraPermissionsDAO implements PermissionsDAO
{
    ...
    public BitSetCassandraPermissionsDAO(String host, String keyspace,
            boolean dropKeyspaceOnInit)
    {
        this(BitSetCassandraPermissionsDAO.buildSession(host), keyspace,
                dropKeyspaceOnInit);
    }
            ...
            boolean dropKeyspaceOnInit)
    {
        ...
        this.getAcl = session.prepare("SELECT * FROM " + keyspace
                + ".acl WHERE source_node_id=?"
                + " AND source_node_version=?"
                + " AND permission=?");
        this.getAuth = session.prepare("SELECT * FROM " + keyspace
                + ".auths WHERE auth_name=?");
