...
public class CassandraPermissionsDAO1
{
	...
	public CassandraPermissionsDAO1(String host, String keyspace, boolean dropKeyspaceOnInit)
	{
		...
		if(keySpaceMetadata == null)
		{
		    ...
		    session.execute("CREATE TABLE " + keyspace + ".node_children (" + "parent_node_id text, "
		    		+ "parent_node_version text, "
		    		+ "permission text, "
		            + "authority_start bigint,"
		            + "authority_end bigint,"
		            + "child_node_id text,"
	    ...
	    this.insertChildAuthority = session.prepare("insert into " + keyspace + ".child_authorities "
	    		+ "(authority, parent_authority, rangeStart, rangeEnd) "
				+ "VALUES(?, ?, ?, ?)");
	    ...
	    this.removeChildAuthority = session.prepare("delete from " + keyspace + ".child_authorities "
	    		+ "WHERE parent_authority = ? AND child_authority = ?");
	    this.getChildPermissions = session.prepare("select * from " + keyspace + ".child_permissions where parent_permission = ?");
	    this.getChildAuthorities = session.prepare("select * from " + keyspace + ".child_authorities where parent_authority = ?");
	    this.getAuthority = session.prepare("select * from " + keyspace + ".child_authorities where authority = ?");
	    this.insertNodeChild = session.prepare("insert into " + keyspace + ".node_children (parent_node_id, parent_node_version, "
	    		+ "permission, authority_start, authority_end, child_node_id, child_node_version) VALUES(?, ?, ?, ?, ?, ?, ?)");
		this.getChildren = session.prepare("SELECT * FROM " + keyspace + ".node_children WHERE parent_node_id=?"
				+ " AND parent_node_version=?"
...
//        Spliterator<Row> spliterator = resultSet.spliterator();
	...
	{
		return Stream.concat(Stream.of(parentAuthority), getChildAuthoritiesAsStream(parentAuthority));
	}
...
//        		.onClose(() -> resultSet.)  // need to close cursor;
    			.flatMap(row ->
    			{
    				...
    				return Stream.concat(Stream.of(childAuthority), getChildAuthoritiesAsStream(childAuthority));
    			});
    	return stream;
	}
	...
	public void addChildAuthority(String parentAuthority, String childAuthority, long rangeStart, long rangeEnd)
	{
		BoundStatement insert = insertChildAuthority.bind(parentAuthority, childAuthority, rangeStart, rangeEnd);
		session.execute(insert);
	}
	...
	public void setPermission(final String parentNodeId, final String parentNodeVersion, final String permission,
			final String authorityName, final String childNodeId, final String childNodeVersion)
	{
		...
		{
			insertNodeChild = session.prepare("insert into " + keyspace + ".node_children (parent_node_id, parent_node_version, "
					+ "permission, authority_start, authority_end, child_node_id, child_node_version) VALUES(?, ?, ?, ?, ?, ?, ?)");
			BoundStatement insert = insertNodeChild.bind(parentNodeId, parentNodeVersion, p, authority.getRangeStart(),
					authority.getRangeEnd(), childNodeId, childNodeVersion);
			session.execute(insert);
		});
	}

	public void setPermissions(String parentNodeId, String parentNodeVersion, Map<String, String> permissions,
			String childNodeId, String childNodeVersion)
	{
		permissions.forEach((permission, authority) ->
		{
			setPermission(parentNodeId, parentNodeVersion, permission, authority, childNodeId, childNodeVersion);
		});
	}

	public Stream<String> getChildren(String parentNodeId, String parentNodeVersion, String authorityName)
	{
		Authority authority = getAuthority(authorityName);

		BoundStatement query = getChildren.bind(parentNodeId, parentNodeVersion, authority.getRangeStart(),
				authority.getRangeEnd());
