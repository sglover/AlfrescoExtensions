...
public class MongoContentDAO implements ContentDAO
{
    ...
    public MongoContentDAO(DB db, String contentCollectionName,
    ...
    private void init()
    {
        ...
        this.contentData = getCollection(db, contentCollectionName,
                WriteConcern.ACKNOWLEDGED);

        {
            DBObject keys = BasicDBObjectBuilder.start("e", 1).add("n", 1)
                    .add("v", 1).add("pri", 1).get();
    ...
    private NodeInfo toNodeInfo(DBObject dbObject)
    {
        ...
        if (dbObject != null)
        {
            ...
            String mimeTypeStr = (String) dbObject.get("m");
            MimeType mimeType = MimeType.INSTANCES.getByMimetype(mimeTypeStr);
            Long size = (Long) dbObject.get("s");

            nodeInfo = new NodeInfo(Node.build().nodeId(nodeId)
                    .nodeInternalId(nodeInternalId).nodeVersion(nodeVersion)
                    .nodePath(nodePath), contentPath, mimeType, "text/plain", size);
        }
...
//    {
//        QueryBuilder queryBuilder = QueryBuilder.start("e")
//                .is(serverIdentity.getId()).and("p").is(nodePath);
...
//    }
    ...
    {
        QueryBuilder queryBuilder = QueryBuilder.start("e")
                .is(serverIdentity.getId()).and("n").is(nodeId).and("v")
    ...
    @Override
    public NodeInfo getByNodeId(String nodeId, long nodeVersion,
            boolean isPrimary)
    {
        QueryBuilder queryBuilder = QueryBuilder.start("e")
                .is(serverIdentity.getId()).and("n").is(nodeId).and("v")
    ...
    public NodeInfo getByNodeId(String nodeId, long nodeVersion, MimeType mimeType)
    {
        QueryBuilder queryBuilder = QueryBuilder
                ...
                .and("pri").is(false)
                .and("mimeType").is(mimeType.getMimetype());
        DBObject query = queryBuilder.get();
    ...
    {
        QueryBuilder queryBuilder = QueryBuilder.start("e")
                .is(serverIdentity.getId()).and("ni").is(nodeInternalId)
                .and("m").is(mimeType);
        DBObject query = queryBuilder.get();
    ...
    public void updateNode(NodeInfo nodeInfo)
    {
        ...
        QueryBuilder queryBuilder = QueryBuilder.start("e").is(cacheServerId)
                .and("n").is(nodeId);
        if (nodeVersion != null)
        {
        ...
        BasicDBObjectBuilder builder = BasicDBObjectBuilder.start(
                "$set",
                BasicDBObjectBuilder.start("m", mimeType.getMimetype())
                        .add("s", size).add("ni", nodeInternalId)
                        .add("c", contentPath).add("p", nodePath)
                        .add("pri", isPrimary).get());
