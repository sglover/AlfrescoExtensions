...
public class MongoSimilarityDAO extends AbstractMongoDAO implements SimilarityDAO
{
    ...
    private void init()
    {
        ...
        this.similarityData = getCollection(db, similarityCollectionName,
    ...
    {
        DBObject dbObject = BasicDBObjectBuilder.start("n1", node1.getNodeId())
                .add("v1", node1.getNodeVersion()).add("n2", node2.getNodeId())
                .add("v2", node2.getNodeVersion()).add("s", similarity).get();
    ...
    public double getSimilarity(Node node1, Node node2)
    {
        ...
        ors.add(QueryBuilder.start("n1").is(node1.getNodeId()).and("v1").is(node1.getNodeVersion())
                .and("n2").is(node2.getNodeId()).and("v2").is(node2.getNodeVersion()).get());
        ors.add(QueryBuilder.start("n1").is(node2.getNodeId()).and("v1").is(node2.getNodeVersion())
                .and("n2").is(node1.getNodeId()).and("v2").is(node1.getNodeVersion()).get());
    ...
    public List<Similarity> getSimilar(Node node)
    {
        ...
        List<DBObject> ors = new LinkedList<>();
        ors.add(QueryBuilder.start("n1").is(node.getNodeId()).and("v1").is(node.getNodeVersion())
                .get());
        ors.add(QueryBuilder.start("n2").is(node.getNodeId()).and("v2").is(node.getNodeVersion())
                .get());
