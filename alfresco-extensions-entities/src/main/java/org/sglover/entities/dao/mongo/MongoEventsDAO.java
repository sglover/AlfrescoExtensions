...
public class MongoEventsDAO {/*extends AbstractMongoDAO implements EventsDAO
{
    ...
    private void init()
    {
        ...
        {
            DBObject keys = BasicDBObjectBuilder.start("ic", 1).add("tx", 1).get();
    ...
    {
        {
            DBObject query = QueryBuilder.start("txnId").is(event.getTxnId()).get();
    ...
    public List<Event> getEventsForTxn(String txnId)
    {
        ...
        DBObject query = QueryBuilder.start("ic").is(true).and("txnId").is(txnId).get();
