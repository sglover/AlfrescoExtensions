...
public class MongoEntitiesDAO extends AbstractMongoDAO implements EntitiesDAO
{
    ...
    private void init() {
        ...
        {
            DBObject keys = BasicDBObjectBuilder.start("n", 1).add("v", 1)
                    .get();
            ...
            for (Entity<String> nameEntity : entities) {
                ...
                for (EntityLocation location : locations) {
                    ...
                    DBObject locDBObject = BasicDBObjectBuilder
                            .start("s", start).add("e", end)
                            .add("p", probability).add("c", context).get();
                    locs.add(locDBObject);
                }

                DBObject dbObject = BasicDBObjectBuilder
                        .start("n", nodeId).add("ni", nodeInternalId)
                        .add("v", nodeVersion).add("t", type)
                        .add(key, nameEntity.getEntity())
                        .add("c", nameEntity.getCount()).add("locs", locs)
                        .get();
    ...
    public Stream<Entity<String>> getNames(Node node) {
        ...
        BasicDBObjectBuilder orderByBuilder = BasicDBObjectBuilder.start("nm",
                1);
    ...
    public Stream<Entity<String>> getOrgs(Node node) {
        ...
        BasicDBObjectBuilder orderByBuilder = BasicDBObjectBuilder.start("o",
                1);
        DBObject orderBy = orderByBuilder.get();
    ...
    public Stream<Node> matchingNodes(EntityType type, String name) {
        ...
        try {
            for (DBObject dbObject : cursor) {
                ...
                Node node = Node.build().nodeInternalId(nodeInternalId)
